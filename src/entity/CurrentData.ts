...
import {ICurrentData, TCurrentDataIn, TCurrentDataOut} from '../types/CurrentData';
import {statusEnum} from "../types/interfaces";
 ...
 *
 * */
export class CurrentData extends GeoEntity implements ICurrentData {
    extra: string;
    tLast: number;
    tValid: number;
    tArc: number;
    ...
    constructor(data: TCurrentDataIn) {
        super();
    ...
    get coordinates() {
        if (![statusEnum.success].includes(this.status)) {
            return {
                lat: this.lbs_lat,
                lng: this.lbs_lng
            };
    ...
    get status() {
        if (!this.isCoordinatesValid) return statusEnum.NoConnection;
        if (this.hDop === 0) return statusEnum.GPSNotSignal;
        if (this.hDop >= 1) return statusEnum.success;
        if (this.hDop < 0) {
            const dop = Math.abs(this.hDop);
            if (dop & 1)
                return statusEnum.GPSSuppression;
            if (dop & 8)
                return statusEnum.GPSReceiverFailure;
        }
        return statusEnum.unknown;
    }

    public get model(): TCurrentDataOut {
        return {
            id: this.id,
            extra: this.extra,
            tLast: this.tLast,
    ...
    private get isCoordinatesValid() {
        const MINUTES = 2;
